const Main = () => {
  ...
  const { selectedSong } = useContext(PlayingContext);
    ...
    function guitar_chord_writer() {
      ...
      this.init = function (transposition_text, up, down) {
        ...
        $(song_selector).each(function (index) {
          ...
          s = s.replace(/\{/g, "<span class='chord light-txt'>");
          s = s.replace(/\}/g, "</span>");
          $(this).html(s);
        });
  ...
  return (
    <main>
      <h3 className="txt-center light-txt">{selectedSong.selectedSongTitle}</h3>
      <h5 className="txt-center">{selectedSong.selectedSongArtist}</h5>
      <div className="song">
        <p className="lyrics txt-center">
          {selectedSong.selectedSongChords}
        </p>
      </div>
    </main>
  );
